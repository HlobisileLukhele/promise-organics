    ...
    <>
          ...
          <div className="flex items-center gap-x-4 text-2xl text-gray-700">
            {user ? (
              <div className="flex items-center gap-2">
                <Link to="/account" className="text-sm font-medium text-gray-700 hidden sm:block hover:text-[#7c8c7d]">
                  Welcome, <span className="text-[#7c8c7d] font-semibold">{user.full_name?.split(' ')[0]}</span>
                </Link>
                <button
                  onClick={logout}
                  className="text-xs text-gray-500 hover:text-[#c8937b] border border-gray-200 px-2 py-1 rounded hidden sm:block cursor-pointer"
                >
                  Logout
                </button>
        ...
        {isMenuOpen && (
          <div className="lg:hidden fixed top-0 right-0 w-64 h-full bg-white shadow-lg z-50 p-5 transition-transform">
            <div className="flex justify-end mb-4">
              <LiaTimesSolid className="text-2xl cursor-pointer hover:text-[#c8937b]" onClick={toggleMenu} />
            </div>
      ...
      {showSearchDrawer && (
        <div className="fixed top-0 left-0 w-full h-full bg-white z-50 flex items-center justify-center flex-col transition-all duration-300">
          <button onClick={toggleSearchDrawer} className="absolute top-4 right-6 text-2xl hover:text-[#c8937b] hover:cursor-pointer">
            <LiaTimesSolid />
          </button>
          <div className="relative w-4/5 max-w-xl">
            <input type="text" placeholder="Search for products..." className="w-full border border-gray-300 rounded-md px-12 py-3 text-lg focus:outline-none" />
      ...
      <div className={`fixed top-0 right-0 h-full w-96 bg-white shadow-lg z-50 p-6 transition-transform duration-300 transform ${showHeartDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        <button onClick={toggleHeartDrawer} className="absolute top-4 right-4 text-xl hover:text-[#c8937b] hover:cursor-pointer">
          <LiaTimesSolid />
        </button>
        <h2 className="text-xl font-semibold mt-14 mb-4">Your Wishlist ({wishlistCount})</h2>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-120px)]">
            {wishlistItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 border-b border-gray-100 pb-4">
                <img src={item.image} alt={item.name} className="w-14 h-16 object-contain bg-gray-50 rounded" />
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#3d4f3e] line-clamp-2">{item.name}</p>
                  <p className="text-sm text-gray-500 mt-1">R{parseFloat(item.price).toFixed(2)}</p>
                </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-96 bg-white shadow-lg z-50 p-6 transition-transform duration-300 transform ${showCartDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        <button onClick={toggleCartDrawer} className="absolute top-4 right-4 text-xl hover:text-[#c8937b] hover:cursor-pointer">
          <LiaTimesSolid />
        </button>
        <h2 className="text-xl font-semibold mt-14 mb-4">Shopping Cart ({cartCount})</h2>
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center text-center mt-8">
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-220px)]">
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 border-b border-gray-100 pb-4">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#3d4f3e] line-clamp-2">{item.name}</p>
                  <p className="text-sm text-gray-500 mt-1">x{item.quantity} · R{(item.price * item.quantity).toFixed(2)}</p>
                </div>
